...
class ObjetosMercader(context: Context):
    SQLiteOpenHelper(context, DATABASE, null, DATABASE_VERSION) {

        companion object{
            private const val DATABASE_VERSION = 1
            private const val DATABASE = "ObjetosMercader.db"
            private const val TABLA_OBJETOS_MERCADER = "objetos_mercader"
            private const val KEY_ID = "_id"
            private const val COLUMN_NOMBRE = "nombre"
            private const val COLUMN_TIPO = "tipo"
            private const val COLUMN_PRECIO = "precio"
            private const val COLUMN_PESO = "peso"
            private const val COLUMN_URI_IMAGEN = "uri"
    ...
    private fun insertarArticulosDefault(db: SQLiteDatabase?) {
        ...
        for (i in listaArticulos){
            ...
            db?.insert(TABLA_OBJETOS_MERCADER, null, values)
        }
    }

    override fun onUpgrade(db: SQLiteDatabase?, oldVersion: Int, newVersion: Int) {
        db?.execSQL("DROP TABLE IF EXISTS $TABLA_OBJETOS_MERCADER")
        onCreate(db)
    }
    ...
    fun insertarArticulo(articulo: Articulos): Long {
        ...
        val id = db.insert(TABLA_OBJETOS_MERCADER, null, values)
        db.close()
        return id
    }
    ...
    fun getArticulos(): ArrayList<Articulos> {
        ...
        val selectQuery = "SELECT * FROM $TABLA_OBJETOS_MERCADER"
